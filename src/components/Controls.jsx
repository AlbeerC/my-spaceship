    ...
    <div className="absolute top-[42%] left-[35%] w-[22%] h-[22%] rounded-lg overflow-hidden">
      ...
      <div className="relative h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mb-1 p-1">
          <div className="text-[12px] font-mono text-cyan-300">SHIP CONTROL INTERFACE</div>
        ...
        <div className="flex-1 flex">
          ...
          <div className="w-[60%] pr-1 border-r border-cyan-500/20 p-1">
            <div className="text-[10px] font-mono text-cyan-400 mb-1">SHIP SYSTEMS</div>
            <div className="grid grid-cols-2 gap-3">
              {[
                { id: "navigation", label: "NAV", color: "pink" },
                { id: "shields", label: "SHIELDS", color: "purple" },
                { id: "engines", label: "ENG", color: "green" },
                { id: "communications", label: "COM", color: "blue" }
              ].map((system) => (
                <motion.button
                  key={system.id}
                  className={`p-2 rounded border text-left text-[10px] font-mono cursor-pointer
                    ${
                      systemStates[system.id]
                        ? `border-${system.color}-500/50 bg-${system.color}-500/10 text-${system.color}-300`
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <motion.div
                      ...
                      animate={{
                        opacity: systemStates[system.id] ? [0.5, 1, 0.5] : 0.5,
                      }}
          ...
          <div className="w-[40%] pl-1 flex flex-col">
            ...
            >
              <div className="text-[10px] font-mono font-bold text-cyan-300">WARP</div>
            ...
            <div className="bg-black/40 rounded p-1">
              <div className="text-[10px] font-mono text-gray-400 mb-0.5">STATUS</div>
              <div className="space-y-0.5">
                <div className="flex justify-between text-[10px] font-mono">
                  <span className={systemStates.engines ? "text-green-400" : "text-red-400"}>
                    ENG:{systemStates.engines ? "ON" : "OFF"}
                  </span>
                  <span className={systemStates.shields ? "text-purple-400" : "text-gray-400"}>
                    SHD:{systemStates.shields ? "ON" : "OFF"}
                  </span>
                </div>
                <div className="flex justify-between text-[10px] font-mono">
                  <span className={systemStates.navigation ? "text-pink-400" : "text-gray-400"}>
                    NAV:{systemStates.navigation ? "ON" : "OFF"}
                  </span>
                  <span className={systemStates.communications ? "text-blue-400" : "text-gray-400"}>
      ...
      {lastActivated && (
        ...
        >
          <div className="text-[8px] font-mono text-cyan-300">
            ...
            {lastActivated === "shields" && `SHIELDS ${systemStates.shields ? "ACTIVATED" : "DEACTIVATED"}`}
            {lastActivated === "navigation" && `NAVIGATION ${systemStates.navigation ? "ONLINE" : "OFFLINE"}`}
            {lastActivated === "engines" && `ENGINES ${systemStates.engines ? "ONLINE" : "OFFLINE"}`}
            {lastActivated === "communications" && `COMMS ${systemStates.communications ? "ACTIVE" : "INACTIVE"}`}
          </div>
        </motion.div>
      )}
    </div>
